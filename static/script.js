    ...
    async function handleReverseSort() {
        try {
            ...
            if (response.ok && result.status === 'ok') {
                // Don't reset position or load a new sujet, just update the sort direction
                // and provide visual feedback that the sort direction has changed
                const sortIcon = reverseSortButton.querySelector('i') || reverseSortButton;
                if (sortIcon) {
                    // Toggle between up and down arrow icons if they exist
                    if (sortIcon.classList.contains('fa-arrow-up')) {
                        sortIcon.classList.replace('fa-arrow-up', 'fa-arrow-down');
                    } else if (sortIcon.classList.contains('fa-arrow-down')) {
                        sortIcon.classList.replace('fa-arrow-down', 'fa-arrow-up');
                    }
                }
                // Provide feedback that sort direction changed
                reverseSortButton.classList.add('active');
                setTimeout(() => reverseSortButton.classList.remove('active'), 500);
            } else {
