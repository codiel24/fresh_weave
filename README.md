...
## Next Steps
- **UI Layout & Wording Cleanup:** Stabilize button positions and improve text/layout for a cleaner interface.
- **Advanced Filtering:** Implement UI controls to filter sujets by tags, people, recency, or other criteria.
- **Database Migration:** Implement a proper database migration system (e.g., using Alembic) to handle future schema changes without data loss.
- **Address `source_file` Column Issue:** Investigate and ensure the `source_file` column is correctly defined and populated.
...
## 4. Current TODO List (Prioritized by recent discussion & existing items)

1.  **Implement Sujet Deletion:** Add a "Delete" button in the UI and corresponding backend logic in `app.py` to remove sujets from `sujets.db` (and consider impact on Google Sheets log - e.g., mark as deleted or remove row?).
2.  **UI Layout & Wording Cleanup:**
    *   Stabilize button positions to prevent them from shifting.
    *   Optimize UI real estate above the "Notes" section (e.g., remove "your enrichment" phrase).
    *   Display "Enrichment Status" and "View Count" more compactly, potentially on the same line.
3.  **Implement Reverse Sort for Sujets:** Add a UI control (e.g., button/toggle) to allow users to reverse the presentation order of sujets.
4.  **Add "Person" Field:** Integrate a "Person" field throughout the application (data model in `sujets.db` via `init_db`, `app.py` logic, `static/script.js` handling, and `templates/index.html` display/input).
5.  **Address `source_file` Column Issue:** Investigate and ensure the `source_file` column is correctly defined and populated in the `sujets` table by `init_db` in `app.py`, or confirm if it's no longer needed. (Related to Memory `9325cdc0-8574-4459-ad38-433e0d9a3692`)
