...
def add_new_sujet(title, ai_suggestion=""):
    """
    ...
    try:
        ...
        db.execute(
            '''INSERT INTO sujets 
               (id, original_sujet, ai_suggestion, user_notes, user_tags, status, view_count, person) 
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
            (next_id, original_sujet, ai_suggestion, "", "", "new", 1, "")
        )
        db.commit()
