...
def update_title(sujet_id):
    ...
    if success:
        ...
        try:
            sujet = db_operations.get_sujet_by_id(sujet_id)
            if sujet:
                log_sujet_to_sheet(sujet, 'title_edited')
...
def add_sujet():
    """Creates a new sujet with the provided title and AI suggestion."""
    data = request.get_json()
    ...
    ai_suggestion = data.get('ai_suggestion', '').strip()
    
    # Add the new sujet to the database
    new_sujet = db_operations.add_new_sujet(title, ai_suggestion)
    
    if new_sujet:
        # Log to Google Sheets if enabled
        try:
            log_sujet_to_sheet(new_sujet, 'created')
        except Exception as e:
